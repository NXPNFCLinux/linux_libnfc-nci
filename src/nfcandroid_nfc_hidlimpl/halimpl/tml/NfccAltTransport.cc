...
{
  unsigned long pinInt = PIN_INT;        // Default fallback values
  unsigned long pinEnable = PIN_ENABLE;
  unsigned long pinFwdnld = PIN_FWDNLD;
  
  // Try to read GPIO pins from config file
  GetNxpNumValue("NXP_GPIO_INT", &pinInt, sizeof(pinInt));
  GetNxpNumValue("NXP_GPIO_ENABLE", &pinEnable, sizeof(pinEnable));
  GetNxpNumValue("NXP_GPIO_FWDNLD", &pinFwdnld, sizeof(pinFwdnld));
  
  NXPLOG_TML_D("Using GPIO pins from config: INT=%lu, ENABLE=%lu, FWDNLD=%lu", 
               pinInt, pinEnable, pinFwdnld);
  
  // Assign IO pins
  iInterruptFd = verifyPin(pinInt, 0, EDGE_RISING);
  if (iInterruptFd < 0) return (NFCSTATUS_INVALID_DEVICE);
  iEnableFd = verifyPin(pinEnable, 1, EDGE_NONE);
  if (iEnableFd < 0) return (NFCSTATUS_INVALID_DEVICE);
  iFwDnldFd = verifyPin(pinFwdnld, 1, EDGE_NONE);
